<template>
	<scroll-view id="content" scroll-y="true" >
			...
			<view id="commentBox" v-show="!hide">
				<view class="comment" v-for="comment in TestData.tests" :key="comment.id">
					...
					<view class="image" v-if="comment.image_list.length != 0">
						<image :src="base_url+image" mode="aspectFill" v-for="image in comment.image_list"></image>
					</view>
				</view>
			</view>
		</view>
		...
		<view id="fifthBlock">
			...
			<view class="comment" v-for="comment in TestData.userComment" :key="comment.id">
				<view class="commentInfo">
					<image :src="base_url+comment.user.avatar" mode="aspectFill"></image>
					<text class="name">{{comment.user.username}}</text>
					<text class="publishTime">{{comment.publish_time}}</text>
					<text class="content">{{comment.comment_text}}</text>
				</view>
			</view>
		</view>
	...
	<view id="inputBox">
		<view id="input">
			<input type="text" placeholder="写写您的看法" v-model="commentText">
		</view>
		<button @click="publish">评价</button>
	</view>
</template>
...
<script setup>
	...
	function publish(){
		if(commentText.value == ""){
			...
			createRequest("/test/addTextComment","POST",{
						'Content-Type': 'application/json',
